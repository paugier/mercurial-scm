...
This was very much the point of this first day: a focus on the project itself
and to care for the many details that get left out of planning meetings.
...
command ([#1973][11]) to create a synthetic repo from a DAG, and some good
...
time for Mercurial to grow its FOSS, fully integrated FUSE.

Upstream development of this effort was started earlier this year. The first
experimental read-only and local version is already being used by real users in
conjunction with an overlay filesystem to support writes. This has improved the
...
This sprint, the discussions were mostly about planning what's next for the
FUSE: faster update still, seamless support in `hg update`, and an integrated
write layer.
...
As soon as you can do multiple things at once, you will have conflicts: they are
an inevitable part of version control and most version control systems give you
neither a good model not a good interface to help you with them.
...
and has helped us on these issues over the past few years.

Of course, lately the [JJ](https://docs.jj-vcs.dev/latest) VCS has become very
popular, with its own flavor of first-class conflicts. A lot of users seem to
...
as there are still a lot of edge cases were the more heuristic approach fails.
...
Augier, and has been a topic of discussion with Patrick Steinhardt for a few
months now.
...
simple use cases (IDEs, shell prompt, AI agents, etc.) thanks to its new FUSE as
detailed above.
...
the revlog, called the `Hyperlog`: the name started out as a joke, but it feels
...
* Multiple index and data files for extensibility and memory locality
...
index. This time, the format will also be different for the changelog,
...
We made a lot of progress in different areas, and had people from many companies
